...
namespace BaseProject.Utilities
{
    public static class APICall
    {
        ...
        public static T PostObject<T>(string url, string type, string token, object values = null)
        {
            T obj;
            ...
            try
            {
                using (HttpClient client = new HttpClient())
                {
                    ...
                    obj = JsonConvert.DeserializeObject<T>(client.PostAsync(url, httpContent).Result.Content.ReadAsStringAsync().Result);
                }
            }
            catch (Exception ex)
            {
